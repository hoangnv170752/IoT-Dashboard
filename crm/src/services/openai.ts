...
async function buildContext(user: JwtPayload): Promise<string> {
  if (!user.tenantId) {
...
async function getConversationHistory(sessionId: string): Promise<OpenAI.Chat.ChatCompletionMessageParam[]> {
...
export function setupChatHandler() {
  onMessage('chat_message', async (socket, message, user) => {
    ...
    const chatMessage = message as unknown as { sessionId?: string; content: string };
...
export async function chatCompletion(
  ...
  content: string,
  user: JwtPayload
): Promise<{ sessionId: string; response: string; messageId: string }> {
  // Get or create session
  const actualSessionId = await getOrCreateSession(sessionId, user);
