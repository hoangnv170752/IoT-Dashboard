...
import { stripe, createCheckoutSession, createPortalSession, handleWebhookEvent, getSubscriptionUsage, checkPlanLimits, } from '../services/stripe.js';
...
export async function billingRoutes(fastify) {
    ...
    fastify.post('/billing/subscription/checkout', {
        ...
        try {
            const session = await createCheckoutSession(request.user.tenantId, priceId, successUrl, cancelUrl);
            return reply.send({
    ...
    fastify.post('/billing/subscription/portal', {
        preHandler: [authenticate, requireTenantAdmin],
        schema: {
            tags: ['Billing'],
        ...
        try {
            const session = await createPortalSession(request.user.tenantId, request.body.returnUrl);
            return reply.send({
                url: session.url,
            });
        }
    ...
    fastify.post('/billing/webhook', {
        ...
        try {
            ...
            const event = stripe.webhooks.constructEvent(typeof rawBody === 'string' ? rawBody : JSON.stringify(rawBody), sig, webhookSecret);
